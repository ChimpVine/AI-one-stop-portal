...
def department(department_name):
    ...
    if department:
        ...
        return render_template(f'{department_name.lower()}.html')
    else:
...
def quiz_json(topic, subject, number, difficulty, grade, description):
    model = ChatOpenAI(
        model="gpt-4o",
...
".
    ...
    query = f"Generate quiz on topic {topic} for subject {subject}. Generate {number} number of questions in {difficulty} mode. This quiz is made for grade {grade} students. The topic description is {description} "
...
def submit_quizhfunForm():
    ...
    for row_number, row_data in all_rows_with_row_numbers:
        ...
        if content:
            ...
            conn.request(
                "POST", "/wp-json/custom/v1/create-quiz", wordpress_payload, headers)
...
def video_script():
    if request.method == 'POST':
        ...
        if sheet_id and folder_id:
            ...
            flash(
                "Please enter both Google Sheet ID and Google Drive Folder ID.", "warning")
    return render_template('Video script.html')
