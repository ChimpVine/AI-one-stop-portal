...
def process_data(sheet_id, folder_id):
    ...
    prompt_template = """Translate the following text into the specified language:
        ...
        try:
            ...
            for row_number, row_data in all_rows_with_row_numbers:
                ...
                if text and language:
                    ...
                    original_audio_content = BytesIO(response.content)

                    # Upload original audio file to Google Drive
                    ...
                    original_media = MediaIoBaseUpload(
                        original_audio_content, mimetype="audio/mpeg")
                    ...
                    translated_audio_content = BytesIO(response.content)

                    # Upload translated audio file to Google Drive
                    ...
                    translated_media = MediaIoBaseUpload(
                        translated_audio_content, mimetype="audio/mpeg")
                    ...
                    # Update sheet with status and timestamps
                    sheet.update_cell(
                        row_number, sheet.find("Status").col, "Success!")
                    sheet.update_cell(row_number, sheet.find("Date").col, date)
                    sheet.update_cell(row_number, sheet.find("Time").col, time)
...
def create_admin():
    if request.method == 'POST':
        ...
        existing_admin = User.query.filter_by(
            email=email, is_admin=True).first()
        if existing_admin:
            flash('Admin with this email already exists.', 'error')
            return redirect(url_for('create_admin'))

        # Create a new admin user
        new_admin = User(email=email, full_name=full_name, is_admin=True)
